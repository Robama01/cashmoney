...
def dashboard():
    try:
        app.logger.info("dashboard session user_id=%s", session.get("user_id"))
        u = get_user_by_id(session["user_id"])
        app.logger.info("dashboard user=%s", u)
        if not u:
            session.clear()
            return redirect(url_for("login"))
...
def not_found(error):
    app.logger.warning("404 sur %s", request.path)
    return "Page introuvable", 404

@app.errorhandler(500)
def server_error(error):
    app.logger.exception("Erreur 500 sur %s", request.path)
    return "Erreur interne du serveur", 500
