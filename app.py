...
GAINS = [D("0.4325"), D("0.3460"), D("0.2595"), D("0.2076"), D("0.1730"),
...
def verify_and_migrate_password(user, password):
    ...
    if stored.startswith("pbkdf2:") or stored.startswith("scrypt:") or stored.startswith("bcrypt:"):
...
def process_payment(user_id, tx_hash):
    with get_db() as conn:
        try:
            ...
            conn.commit()
            app.logger.info("Paiement traité user_id=%s tx=%s", user_id, tx_hash)
            return True
        except Exception:
            conn.rollback()
    ...
    try:
        if request.method == "POST":
            ...
            with get_db() as conn:
                ...
                conn.commit()
            app.logger.info("Inscription email=%s", email)
            ...
            if ok:
                if new_hash:
                    with get_db() as conn:
                        ...
                        conn.commit()
                    app.logger.info("Mot de passe migré user_id=%s", u["id"])
                session["user_id"] = u["id"]
                app.logger.info("Connexion réussie email=%s user_id=%s", email, u["id"])
                return redirect(url_for("dashboard"))

            app.logger.info("Connexion échouée email=%s", email)
            flash("Email ou mot de passe incorrect.", "error")
            return render_template("login.html")
...
def dashboard():
    try:
        ...
        app.logger.exception("Erreur dashboard user_id=%s", session.get("user_id"))
...
def depot():
    try:
        ...
        app.logger.exception("Erreur depot user_id=%s", session.get("user_id"))
        return render_template("error.html", message="Erreur interne"), 500
...
def test_pay(uid):
    try:
        ...
        app.logger.exception("Erreur test_pay uid=%s", uid)
...
def watch_ads():
    try:
        ...
        app.logger.exception("Erreur watch_ads user_id=%s", session.get("user_id"))
        return render_template("error.html", message="Erreur interne"), 500
...
def claim_reward():
    try:
        ...
        app.logger.exception("Erreur claim_reward user_id=%s", session.get("user_id"))
