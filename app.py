...
<p style="text-align:center;margin-top:1rem"><a href="/admin/reset-password" style="color:#f0b429">Mot de passe oublié ?</a></p>
</div></div>
<script>
...
def add_video():
    ...
    # Si un montant investi est fourni, la récompense nette est calculée automatiquement
    if investment_amount > 0:
        reward_amount = investment_amount - maintenance_fee
        if reward_amount < 0:
    ...
    cur = conn.cursor()
    cur.execute(
        """INSERT INTO videos (youtube_id, title, sponsor_name, reward_amount, min_watch_seconds, investment_amount, maintenance_fee)
           VALUES (%s, %s, %s, %s, %s, %s, %s) RETURNING *;""",
        (data.get("youtube_id"), data.get("title"), data.get("sponsor_name"),
         reward_amount, data.get("min_watch_seconds", 30), investment_amount, maintenance_fee)
    )
    video = cur.fetchone()
    conn.commit()
    cur.close()
...
<input id="investment" type="number" step="0.01" value="0.90" oninput="updateNetReward()">
<label>Frais d'entretien du site (USDT)</label>
<input id="maintenanceFee" type="number" step="0.01" value="0.20" oninput="updateNetReward()">
<label>Récompense nette versée aux utilisateurs (calcul automatique)</label>
<input id="reward" type="number" step="0.01" readonly style="opacity:.7">
...
<script>
function updateNetReward(){{
  const investment = parseFloat(document.getElementById('investment').value) || 0;
  const fee = parseFloat(document.getElementById('maintenanceFee').value) || 0;
  document.getElementById('reward').value = (investment - fee).toFixed(2);
}}
updateNetReward();
...
async function addVideo(){{
  ...
  const msg = document.getElementById('msg');
  if(data.id){{
    msg.innerHTML = '<div class="alert success">Vidéo ajoutée ! Récompense nette : ' + data.reward_amount + ' USDT (frais d\\'entretien prélevés : ' + data.maintenance_fee + ' USDT)</div>';
