...
def init_db():
 conn=get_db()
 c=conn.cursor()
 c.execute("CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT,email TEXT UNIQUE,password_hash TEXT,nom TEXT,parrain_id INTEGER,actif INTEGER DEFAULT 0,gains_total REAL DEFAULT 0,wallet_sender TEXT,date_inscription TEXT)")
 c.execute("CREATE TABLE IF NOT EXISTS paiements (id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER,montant REAL,statut TEXT DEFAULT 'en_attente',tx_hash TEXT UNIQUE,wallet_sender TEXT,date_confirmation TEXT,date TEXT)")
 c.execute("CREATE TABLE IF NOT EXISTS commissions (id INTEGER PRIMARY KEY AUTOINCREMENT,beneficiaire_id INTEGER,source_id INTEGER,niveau INTEGER,montant REAL,date TEXT)")
 conn.commit()
 conn.close()
def hp(p):
...
def dist(uid):
 ...
 cur=uid
 for _ in range(12):
  r=conn.execute("SELECT parrain_id FROM users WHERE id=?",(cur,)).fetchone()
...
def inscription():
 ...
 if request.method=="POST":
  ...
  if pc:
   conn=get_db()
   p=conn.execute("SELECT id FROM users WHERE id=?",(pc,)).fetchone()
   conn.close()
   if p:pid=p["id"]
  conn=get_db()
  conn.execute("INSERT INTO users (nom,email,password_hash,parrain_id,date_inscription) VALUES (?,?,?,?,?)",(nom,email,hp(pw),pid,datetime.now().strftime("%Y-%m-%d %H:%M")))
  conn.commit()
  conn.close()
...
def dashboard():
 ...
 conn=get_db()
 f=conn.execute("SELECT COUNT(*) as n FROM users WHERE parrain_id=? AND actif=1",(u["id"],)).fetchone()["n"]
 c=conn.execute("SELECT * FROM commissions WHERE beneficiaire_id=? ORDER BY date DESC LIMIT 10",(u["id"],)).fetchall()
 p=conn.execute("SELECT * FROM paiements WHERE user_id=? AND statut='en_attente'",(u["id"],)).fetchone()
 conn.close()
 lien=request.host_url+"inscription?ref="+str(u["id"])
 return render_template("dashboard.html",user=u,filleuls=f,commissions=c,pending=p,lien_parrainage=lien,wallet=WALLET_USDT,membership=MEMBERSHIP_USDT,gains_niveau=GAINS)
...
def admin_dashboard():
 ...
 pp=conn.execute("SELECT p.*,u.nom,u.email FROM paiements p JOIN users u ON u.id=p.user_id WHERE p.statut='en_attente'").fetchall()
